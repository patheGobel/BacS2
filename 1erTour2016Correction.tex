...
\begin{tikzpicture}
 \tkzTabInit{$x$/1,$f'$/1,$f$/2}{$-1$,$-\frac{1}{2}$,$+\infty$}
 \tkzTabLine{,+,z,-,}
...
%\[ \forall x \in \left( ]-1;\alpha[\cup]\beta;+\infty[\right),\quad g(x)<0 \text{ donc } -xg(x)<0 \]

%\[ \forall x \in \left( ]\alpha;\beta[\right),\quad g(x)>0 \]
%dresser son tableau de variations.$\textbf{1 pt}$
\end{enumerate}
